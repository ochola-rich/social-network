...
// src/app/notifications/page.tsx
// Notifications page — displays real-time notifications and pending follow requests
// with actionable Confirm/Decline buttons for follow requests.
// Zero hardcoded data. All content fetched from backend.
...
export default function NotificationsPage() {
  ...
  const handleAcceptFromNotification = async (relatedUserId: number) => {
    try {
      await apiClient.post(`/api/follow/accept/${relatedUserId}`);
      fetchNotifications();
      fetchFollowRequests();
    } catch (err) {
  ...
  const handleDeclineFromNotification = async (relatedUserId: number) => {
    try {
      await apiClient.post(`/api/follow/decline/${relatedUserId}`);
      fetchNotifications();
      fetchFollowRequests();
    } catch (err) {
      console.error("Failed to decline follow:", err);
    }
  };
        ...
        <div className="space-y-3">
          {items.map((item) => (
            ...
            >
              ...
              {/* Show Confirm/Decline buttons for follow_request notifications */}
              {item.type === "follow_request" && (
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={async (e) => {
                      e.stopPropagation();
                      await handleAcceptFromNotification(item.related_user_id);
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={async (e) => {
                      e.stopPropagation();
                      await handleDeclineFromNotification(item.related_user_id);
                    }}
                  >
                    <UserX size={14} /> Decline
                  </Button>
