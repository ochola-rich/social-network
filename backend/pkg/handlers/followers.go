...
func (h *FollowHandler) SendFollowRequest(w http.ResponseWriter, r *http.Request) {
	...
	err = h.db.QueryRow("SELECT COUNT(*) FROM users WHERE id = ?", targetID).Scan(&targetExists)
	...
	// ALWAYS create a pending request, regardless of profile privacy
	if err == sql.ErrNoRows {
		_, err = h.db.Exec("INSERT INTO followers (follower_id, followee_id, status) VALUES (?, ?, 'pending')", currentUserID, targetID)
	} else {
		_, err = h.db.Exec("UPDATE followers SET status = 'pending' WHERE follower_id = ? AND followee_id = ?", currentUserID, targetID)
	}

	if err != nil {
		log.Printf("Error updating follow status: %v", err)
		http.Error(w, `{"error":"database error updating follow status"}`, http.StatusInternalServerError)
		return
	}
...
func (h *FollowHandler) AcceptFollowRequest(w http.ResponseWriter, r *http.Request) {
	...
	// FIX: SEND NOTIFICATION BACK TO FOLLOWER THAT THEY WERE ACCEPTED
	if h.notifications != nil {
		h.notifications.CreateNotification(followerID, "follow", "accepted your follow request", currentUserID, 0)
	}
